...
unique(birds_1$species_id) # pipe operator used instead of $
...
```{r, include = FALSE}
birds_df <- birds_1 %>% 
  left_join(taxalist_1, by = "species_id") # wrong key used, not species but species_id is the name of the col
```
...
bird_totals_by_park <- birds_df %>% 
  left_join(sites_1) %>% 
  group_by(park_name) %>% # function mistyped, missing an _
...
ggplot(data = bird_totals_by_park, aes(x = total_count, y = park_name)) +
  ...
  labs(x = "Total bird count",
       y = "Park name") + # pipe operator used instead of +
  theme_minimal()
```
...
common_birds <- birds_df %>% 
  left_join(sites_1) %>% # should not be semi_join(), need to add park_names column to birds_df
...
```{r}
ggplot(data = selected_parks, aes(y = common_name, x = total_count)) + # x and y were not within aes()
  geom_col() +
